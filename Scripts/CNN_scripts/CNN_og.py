"""
CNN Model for Leukemia Classification - Combined Training Data
Input: 450x450x1 Grayscale images (Healthy vs Leukemia cells)
...
class LeukemiaCNN(nn.Module):
    """
    Recommended CNN Architecture for 450x450 grayscale images
    
    Architecture reasoning:
    - Input: 450x450x1 → We have high-resolution medical images in grayscale
    - Strategy: Progressive downsampling with increasing channels  
    - Goal: Extract cell morphology features for classification
    - CHANGED: Input channels from 3 (RGB) to 1 (Grayscale)
    """
    ...
    def __init__(self, num_classes=2, dropout_rate=0.5):
        ...
        # CHANGED: Input channels from 3 to 1 for grayscale
    ...
    def forward(self, x):
        ...
        Args:
            x: Input tensor of shape (batch_size, 1, 450, 450)  # CHANGED: 3→1 for grayscale
...
def preparar_dataloaders_todos_los_folds(datasets, labels, batch_size=32, val_size=0.2):
    ...
    print("  - Convirtiendo imagenes a escala de grises...")
    ...
    print(f"\nPreparando DataLoaders con TODOS los folds combinados:")
    print(f"  - Total imagenes: {len(X)}")
    print(f"  - Shape de imagenes: {X[0].shape}")  # Should be (450, 450, 1)
...
def medir_tiempo_inferencia(model, batch_size=32, num_iterations=50):
    ...
    # Input shape for grayscale (1 channel instead of 3)
...
def analizar_rendimiento_computacional(model, metrics, output_path="logs_combined"):
    """
    Comprehensive computational and performance analysis
    """
    ...
    all_metrics = {
        ...
        'configuracion': 'GRAYSCALE_450x450x1_COMBINED'
    }
...
def generar_reporte_completo(metrics, output_path, timestamp):
    ...
    """
    filename = f"reporte_rendimiento_combined_{timestamp}.txt"
    filepath = os.path.join(output_path, filename)
    
    with open(filepath, 'w') as f:
        f.write("=" * 80 + "\n")
        f.write("REPORTE COMPLETO DE RENDIMIENTO - CNN LEUKEMIA CLASSIFICATION\n")
        f.write("=" * 80 + "\n\n")
        
        f.write(f"Timestamp: {metrics['timestamp']}\n")
        f.write(f"Modelo: LeukemiaCNN - Grayscale\n")
...
def generar_graficas_entrenamiento(metrics, output_path, timestamp):
    ...
    plt.title('Evolucion de la Perdida durante el Entrenamiento', fontsize=14, fontweight='bold')
    plt.xlabel('Epoca')
    plt.ylabel('Perdida')
    plt.legend()
    ...
    plt.title('Evolucion del Accuracy durante el Entrenamiento', fontsize=14, fontweight='bold')
    plt.xlabel('Epoca')
    plt.ylabel('Accuracy (%)')
    plt.legend()
    plt.grid(True, alpha=0.3)
    ...
    plt.title('Tiempo por Epoca', fontsize=14, fontweight='bold')
    plt.xlabel('Epoca')
    ...
    plt.title('Metricas Combinadas', fontsize=14, fontweight='bold')
    plt.xlabel('Epoca')
    plt.legend()
    plt.grid(True, alpha=0.3)
    
    plt.tight_layout()
    plt.savefig(os.path.join(output_path, f'graficas_entrenamiento_{timestamp}.png'), dpi=300, bbox_inches='tight')
    plt.close()
    
    print(f"Graficas de entrenamiento guardadas")
...
def main():
    """
    Main execution function - Combined training with all folds
    """
    print("INICIANDO ENTRENAMIENTO CNN")
    print(f"{'='*60}")
    
    # Hyperparameter configuration (NO OPTUNA)
    hyperparams = {
        'lr': 0.001,
        'dropout_rate': 0.5,
        'batch_size': 32,
    ...
    all_metrics = analizar_rendimiento_computacional(model_trained, metrics, "logs_combined_training")
    
    print(f"\nENTRENAMIENTO Y ANALISIS COMPLETADOS EXITOSAMENTE!")
    print(f"Configuracion: Grayscale 450x450x1 - Datos Combinados")
    print(f"Epochs: {hyperparams['epochs']}")
    print(f"Accuracy final validacion: {metrics['val_accs'][-1]:.2f}%")
    print(f"Loss final validacion: {metrics['val_losses'][-1]:.4f}")
    print(f"Reportes guardados en: logs_combined_training/")
